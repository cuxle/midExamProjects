...
class RequestType(Enum):
    RequestLoginIn = 1
    RequestStudentInfo = 2
    RequestStudentScore = 3
    RequestTypeInvalid = 4
...
class Ui_MainWindow(object):
    ...
    def readJson(self):
        with open('data.json') as f:
            data = json.load(f)
        self.year = data["year"]
        self.m_baseAddress = data["baseAddress"]
    ...
    def setupUi(self, MainWindow):
        ...
        #self.leStudentZkh.editingFinished.connect(self.requestStarted)

        self.pbPrintScore.setDisabled(True)
    ...
    def initPngPath(self):
        ...
        if not dir.exists():
            dir.mkdir(".")
    def makeRequestStudentInfo(self):
    ...
    def requestStarted(self):
        # 1. login request . 2. then request student info. 3. request student score
        print("edit finished 111")
        self.makeLoginRequest()



    def makeLoginRequest(self) -> None:
        json = QJsonDocument()
        obj = json.object()
        ...
        json.setObject(obj)
        print(json.toJson())
        url = self.m_baseAddress + "xuetong/business/biz/host/login"

        request = QNetworkRequest()
        request.setHeader(QNetworkRequest.ContentTypeHeader, "application/json;charset=UTF-8")
        request.setUrl(QUrl(url))
        self.m_requestType = RequestType.RequestLoginIn
        print(url)
    ...
    def requestStudentSocre(self):
        # 2.
        #   2.1 if success get data
        #   2.2 if fail report error
    ...
    def requestFinished(self, reply):
        ...
        if self.m_requestType == RequestType.RequestLoginIn:
            print("request login response")
            data = obj["data"].toObject()
            self.loginToken = data["token"].toString()
            # log in success then get student info
            self.makeRequestStudentInfo()
        elif self.m_requestType == RequestType.RequestStudentInfo:
            data = obj["data"].toObject()
    ...
    def makeHtml(self):
        ...
        htmlStr += '<br>'
        htmlStr += '<br>'
        htmlStr += '<h1 align="center" style=" margin-top:12px; margin-bottom:12px; margin-left:0px; margin-right:0px; -qt-block-indent:0; text-indent:0px;"><span style=" font-size:20pt; font-weight:600;">'+ self.year + '年苏州市初中毕业生</span></h1>'
