...
void FormFuncChoose::initTimers()
{
    ...
    if (!m_cmdOnline) {
        ...
        // do not need mp3 sound when skip not online, 2022.3.8
...
//    m_skipRopeZeroMq->resetCount();
    ...
    // do not need mp3 sound when skip not online, 2022.3.8
    // when offline m_backCountTimer start when detect the first jump
    if (m_cmdOnline) {
...
void FormFuncChoose::initMediaPlayer()
{
    if (m_enableStartSound) {
        ...
        m_mp3Player->setSource(QUrl(m_currentMediaFile));
...
//    connect(m_videoCaptureThread, &QThread::finished, m_videoCaptureThread, &QThread::deleteLater);
    ...
    if (m_camera->isOpencvCam()) {
        ...
        connect(m_camera, &Camera::sigImageCaptureMat, [&](const cv::Mat &mat){
            emit sigImageCaptureMat(mat, m_curTimeLeftMs / 1000); // add second text for TIAOSHENG
        });
    } else {
        ...
        connect(m_camera, &Camera::sigImageCapture, [&](const QImage &image){
            emit sigImageCapture(image, m_curTimeLeftMs / 1000); // add second text for TIAOSHENG
        });
    }
