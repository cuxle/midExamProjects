#include "lidarBasketballAnalysis.h"
#include "appconfig.h"
#include "singleton.h"
#include <QDebug>
...
float lidarBasketballAnalysis::dist(PointXYZ pt1, PointXYZ pt2)
{
	float distance = sqrt((pt1.x - pt2.x)*(pt1.x - pt2.x) + (pt1.y - pt2.y)*(pt1.y - pt2.y));
	return distance;
}

bool lidarBasketballAnalysis::normalizeData(PointCloud<PointXYZ>::Ptr cloudPtr)
{
	...
	for (int i = 0; i < cloudPtr->points.size(); i++)
	{
		if (cloudPtr->points[i].x > m_xMin && cloudPtr->points[i].x < m_xMax && cloudPtr->points[i].y > m_yMin && cloudPtr->points[i].y < m_yMax && cloudPtr->points[i].z > m_zMin && cloudPtr->points[i].z < m_zMax)
		{
			...
			if (dist(tempXYZ, m_flag1) > m_radius && dist(tempXYZ, m_flag2) > m_radius && dist(tempXYZ, m_flag3) > m_radius && dist(tempXYZ, m_flag4) > m_radius && dist(tempXYZ, m_flag5) > m_radius && dist(tempXYZ, m_flag6) > m_radius)
			{
...
/*
bool lidarBasketballAnalysis::removePtsOutOfRegion(float xMin, float xMax, float yMin, float yMax, vector<float> pts)
{
	return true;
}
*/
...
int *lidarBasketballAnalysis::rand_rgb() {//随机产生颜色
	int *rgb = new int[3];
	rgb[0] = rand() % 255;
	rgb[1] = rand() % 255;
	rgb[2] = rand() % 255;
...
//重置考试参数
bool lidarBasketballAnalysis::resetExamParams()
{
	m_prevRegionID = 0;
	...
	m_flagStatus[5] = false;
	return true;
}

//设置考试开始
bool lidarBasketballAnalysis::setExamStart(float x, float y)
{
	//如果标志位已经是考试中，则无需设置
	if (m_inExam == true)
	{
...
//设置考试结束标志
bool lidarBasketballAnalysis::setExamEnd()
{
	//如果标志位已经是考试结束，则无需设置
	if (m_inExam == false)
	{
