...
ExamRegin::ExamRegin(QWidget *parent)
    : QWidget(parent)
{
...
void ExamRegin::updateStudentPointPosFromStdFootGround(std::vector<pcl::PointXYZ> &objs)
{
    // x, y
...
QPointF ExamRegin::calculatNextPoint(const QPointF &p1, const QPointF &p2)
{
    float x = 2 * p2.x() - p1.x();
    float y = 2 * p2.y() - p1.y();
...
void ExamRegin::startExam(bool started)
{
...
void ExamRegin::savePath(const QString &zkh, const QString &time)
{
     // 停止考试了， 要保存考生路径为图片
...
void ExamRegin::updateStudentPointPos(float x, float y)
{
    m_studentPos.setX(x);
    m_studentPos.setY(y);
//    this->update();
...
void ExamRegin::calculateObsStickPosition()
{
...
void ExamRegin::updateRectPointTopLeft(const QPointF &topLeft)
{
    m_topLeft = topLeft;
...
void ExamRegin::updateRectPointBottomRight(const QPointF &bottomRight)
{
...
void ExamRegin::updateRectPoint(const QPointF &topLeft, const QPointF &bottomRight)
{
...
void ExamRegin::showExamStickPos()
{
    QPen m_pen;
    m_pen.setWidth(3);
...
void ExamRegin::setLidarAnalysis(lidarBasketballAnalysis *newLidarAnalysis)
{
    m_lidarAnalysis = newLidarAnalysis;
}

void ExamRegin::setStuMovePathFileName(const QString &newStuPathFileName)
{
    m_stuMovePathFileName = newStuPathFileName;
}

void ExamRegin::showExamStudentPath()
{
    QPen m_pen2;
...
void ExamRegin::showExamStudentPoints()
{
    QPen m_pen;
...
//void ExamRegin::mousePressEvent(QMouseEvent *event)
//{
//    m_stuPointsPath.clear();
//    isLoging = true;
...
//void ExamRegin::mouseReleaseEvent(QMouseEvent *event)
//{
//    if (isLoging) {
//        QPixmap pix(this->size());
//        this->render(&pix);
...
//void ExamRegin::mouseMoveEvent(QMouseEvent *event)
//{
//    if (isLoging) {
